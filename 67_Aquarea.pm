...
# Modul zum belauschen der Kommunikation Kabelfernbedienung >> Panasonic WH-MDC05F3E5 Wärmepumpe
# Vorraussetzung: Baudrate eines ELV RS232-USB Wandlers UM2102 ändern auf 960 Baud, um den Datenbus einer 
# Panasonic Wärmepumpe WH-MDC05F3E5 (Geisha) zu belauschen.
...

{
  ...
  return "" if((length($Aquareadata))<8);
	...
	my @array = split("", $msg);
	 ...
	 if ((ord($array[1]))==17 && $hash->{w_write}==1) # 18 wird ausgeblendet und stattdessen der Befehl gesendet
	 {
			...
			select(undef, undef, undef, 0.035);	# 35ms warten, damit der gesendete Befehl da kommt, wo sonst der FB-Befehl käme
	    my $setrts = $hash->{USBDev}->rts_active(0); # rts fur auf high
  ...
  if($dec[1] != $dec[5]) # wenn die beiden Register von Anfrage und Antwort nicht übereinstimmen return
		{
    	$hash->{PARTIAL} = "";
			return "";
		}
...
#####################################################################################
      	...
      	Aq_readingsSingleUpdate($hash,"019-Vorlauftemperatur",$ergebnis, 1);
			...
			if ($dec[1]==20)	# aktueller Fehler, geht wieder auf 0 wenn fehler behoben dez 38=Fehler H72(Speichertemperaturfühler)
			{
				if ($dec[6] == 38) {$ergebnis="H72(Speichertemperaturfühler)";}
      	...
      	Aq_readingsSingleUpdate($hash,"020-aktueller_Fehler",$ergebnis, 1);
			...
			if ($dec[1]==21)	# Rücklauftemperatur
			{
      	...
      	Aq_readingsSingleUpdate($hash,"023-Kompressorfrequenz",$ergebnis, 1);
			...
			if ($dec[1]==24)	# aktueller Fehler, geht wieder auf 0 wenn fehler behoben dez 38=Fehler H72(Speichertemperaturfühler)
			{
				if ($dec[6] == 38) {$ergebnis="H72(Speichertemperaturfühler)";}
				if ($dec[6] == 156) {$ergebnis="H76(Kommunikationsfehler der Fernbedienung)";}
        ...
        if ($dec[6] & 32) {$ergebnis=$ergebnis." Auto";}
      	...
      	Aq_readingsSingleUpdate($hash,"30-29-Energie_heizen_in_W",(($dec[6]*256)+$hash->{helper}{29}), 1);
			...
			if ($dec[1]==31)	# ??? lowByte Energie kühlen ???
			{
      	# Aq_readingsSingleUpdate($hash,"31",$ergebnis, 1);
				$hash->{helper}{31}=$dec[6];
    	}
			if ($dec[1]==32)	# ??? highByte Energie kühlen ???
			{
      	...
      	Aq_readingsSingleUpdate($hash,"36",$ergebnis, 1);
			...
			if ($dec[1]==129)	# Reset Error (Taste an der Fernbedienung wurde gedrückt)
			{
      	Aq_readingsSingleUpdate($hash,"129-Reset_Error",$ergebnis, 1);
    	}
			if ($dec[1]==130)	# Einstellen der niedrigen Außentemperatur  -15 bis +15
			{
				if ($dec[6] & 128) {$ergebnis=$ergebnis-256;}
      	Aq_readingsSingleUpdate($hash,"130-Heat_On_Out_Lo",$ergebnis, 1);
    	}
			if ($dec[1]==131)	# Einstellen der hohen Außentemperatur  -15 bis +15 
			{
				if ($dec[6] & 128) {$ergebnis=$ergebnis-256;}
      	Aq_readingsSingleUpdate($hash,"131-Heat_On_Out_Hi",$ergebnis, 1);
    	}
			if ($dec[1]==132)	# Einstellen der Wasseraustrittstemperatur bei niedriger Außentemperatur   25 bis 55
			{
				if ($dec[6] & 128) {$ergebnis=$ergebnis-256;}
      	Aq_readingsSingleUpdate($hash,"132-Heat_On_H20_Lo",$ergebnis, 1);
    	}
			if ($dec[1]==133)	# Einstellen der Wasseraustrittstemperatur bei hoher Außentemperatur      25 bis 55
			{
				if ($dec[6] & 128) {$ergebnis=$ergebnis-256;}
      	Aq_readingsSingleUpdate($hash,"133-Heat_On_H20_Hi",$ergebnis, 1);
    	}
			if ($dec[1]==134)	# Einstellen der Außentemperatur, bei der der Heizbetrieb in der Heizbetriebsart abgeschaltet wird   5 bis 35
			{
				if ($dec[6] & 128) {$ergebnis=$ergebnis-256;}
      	Aq_readingsSingleUpdate($hash,"134-Heat_Off_Set",$ergebnis, 1);
    	}
			if ($dec[1]==135)	# Einsteller der Außentemperatur, bei der die Elektrozusatzheizung eingeschaltet wird 
			{
				if ($dec[6] & 128) {$ergebnis=$ergebnis-256;}
      	Aq_readingsSingleUpdate($hash,"135-Heater_On_Out_Set",$ergebnis, 1);
			...
			if ($dec[1]==137)	# Heißwasserspeichertemperatur  40 bis 75 
			{
				if ($dec[6] & 128) {$ergebnis=$ergebnis-256;}
